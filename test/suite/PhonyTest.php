...
class PhonyTest extends TestCase
{
    ...
    {
        $actual = Phony::mockBuilder('Eloquent\Phony\Kahlan\Test\TestClassA');
        ...
        $this->assertInstanceOf('Eloquent\Phony\Kahlan\Test\TestClassA', $actual->get());
    }

    public function testMockBuilderFunction()
    {
        $actual = mockBuilder('Eloquent\Phony\Kahlan\Test\TestClassA');
    ...
    {
        $types = array('Eloquent\Phony\Kahlan\Test\TestClassB', 'Countable');
        ...
        $this->assertInstanceOf('Eloquent\Phony\Mock\Mock', $actual->get());
        $this->assertInstanceOf('Eloquent\Phony\Kahlan\Test\TestClassB', $actual->get());
    ...
    {
        $actual = Phony::spyGlobal('sprintf', 'Eloquent\Phony\Kahlan\Facade');

        $this->assertInstanceOf('Eloquent\Phony\Spy\SpyVerifier', $actual);
        $this->assertSame('a, b', \Eloquent\Phony\Kahlan\Facade\sprintf('%s, %s', 'a', 'b'));
        $this->assertTrue((bool) $actual->calledWith('%s, %s', 'a', 'b'));
    }

    public function testSpyGlobalFunction()
    {
        $actual = spyGlobal('vsprintf', 'Eloquent\Phony\Kahlan\Facade');

        $this->assertInstanceOf('Eloquent\Phony\Spy\SpyVerifier', $actual);
        $this->assertSame('a, b', \Eloquent\Phony\Kahlan\Facade\vsprintf('%s, %s', array('a', 'b')));
    ...
    {
        $actual = Phony::stubGlobal('sprintf', 'Eloquent\Phony\Kahlan\Facade');
        $actual->with('%s, %s', 'a', 'b')->forwards();

        $this->assertInstanceOf('Eloquent\Phony\Stub\StubVerifier', $actual);
        $this->assertSame('a, b', \Eloquent\Phony\Kahlan\Facade\sprintf('%s, %s', 'a', 'b'));
        $this->assertNull(\Eloquent\Phony\Kahlan\Facade\sprintf('x', 'y'));
        $this->assertTrue((bool) $actual->calledWith('%s, %s', 'a', 'b'));
    }

    public function testStubGlobalFunction()
    {
        $actual = stubGlobal('vsprintf', 'Eloquent\Phony\Kahlan\Facade');
        $actual->with('%s, %s', array('a', 'b'))->forwards();

        $this->assertInstanceOf('Eloquent\Phony\Stub\StubVerifier', $actual);
        $this->assertSame('a, b', \Eloquent\Phony\Kahlan\Facade\vsprintf('%s, %s', array('a', 'b')));
        $this->assertNull(\Eloquent\Phony\Kahlan\Facade\vsprintf('x', 'y'));
        $this->assertTrue((bool) $actual->calledWith('%s, %s', array('a', 'b')));
    }

    public function testRestoreGlobalFunctions()
    {
        Phony::stubGlobal('sprintf', 'Eloquent\Phony\Kahlan\Facade');
        Phony::stubGlobal('vsprintf', 'Eloquent\Phony\Kahlan\Facade');

        $this->assertNull(\Eloquent\Phony\Kahlan\Facade\sprintf('%s, %s', 'a', 'b'));
        $this->assertNull(\Eloquent\Phony\Kahlan\Facade\vsprintf('%s, %s', array('a', 'b')));
    ...
    {
        stubGlobal('sprintf', 'Eloquent\Phony\Kahlan\Facade');
        stubGlobal('vsprintf', 'Eloquent\Phony\Kahlan\Facade');
    ...
    private function assertSpyAssertionRecorder($spy)
    {
        ...
        $this->assertInstanceOf('Eloquent\Phony\Kahlan\KahlanAssertionRecorder', $assertionRecorder);
    }
